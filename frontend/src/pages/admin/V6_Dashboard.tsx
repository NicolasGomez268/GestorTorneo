...
const CARDS = [
  {
    ...
    desc:  'Configurá el torneo, divisiones y parámetros generales.',
    cta:   'ENTRAR',
    icon:  <TournamentIcon />,
  },
  {
    ...
    desc:  'Administrá plantillas y jugadores de cada división.',
    cta:   'ADMINISTRAR',
    icon:  <TeamsIcon />,
  },
  {
    to:    '/admin/fixture',
    label: 'Armar Fixture',
    desc:  'Generá el calendario de cruces, fechas y horarios.',
    cta:   'ORGANIZAR',
    icon:  <FixtureIcon />,
  },
  {
...
export default function V6_Dashboard() {
  ...
  return (
    <div>
      ...
      {/* Bienvenida */}
      <div className="mb-8">
        <p className="text-[#FF6B00] text-[10px] font-black tracking-[0.25em] uppercase mb-1">
          Panel de control
        </p>
        <h1 className="text-white font-black text-3xl lg:text-4xl italic uppercase leading-none">
          Bienvenido,<br />
          <span className="text-[#888]">Director de Torneo</span>
        </h1>
      </div>
        ...
        {CARDS.map((card) => (
          ...
          >
            <div className="flex items-start justify-between gap-3 mb-4">
              ...
              {card.mesa && (
                <span className="text-[10px] font-black tracking-widest text-[#888] border border-[#2A2A2A] px-2 py-0.5">
                  TABLET
                </span>
              )}
            </div>
      ...
      <div className="bg-[#FF6B00] p-5 mb-6">
        <p className="text-black/60 text-[10px] font-black tracking-[0.2em] uppercase mb-3">
          Estado general
        </p>
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            <p className="text-black font-black text-4xl font-tabular leading-none">{pendientes}</p>
      ...
      <div>
        ...
        <div className="flex flex-col gap-0">
          {ACTIVIDAD.map((a, idx) => (
            <div
              key={idx}
              className={`flex items-start gap-3 py-3 ${idx < ACTIVIDAD.length - 1 ? 'border-b border-[#1A1A1A]' : ''}`}
            >
